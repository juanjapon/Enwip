...
import server.entities.Nivel_Inversion;
import server.session.FormularioEnwipRemote;

public class CargarNivelInversionFormulario extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CargarNivelInversionFormulario() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      System.out.println("service()...");
		...
		Vector<Nivel_Inversion> nInversion;
		nInversion=str.getNivelInversion();
		

		StringBuilder objSecciones;
         // Crea un array de objetos JSON con cada tema de la tabla
            objSecciones = new StringBuilder("[");
           for( int i=0;i<nInversion.size();i++ )
            { objSecciones.append("{\"Nivel\":\"");
            objSecciones.append(nInversion.get(i).getRango());
            objSecciones.append("\"},");
            }            
