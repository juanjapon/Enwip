...
import server.entities.Tipo_Inversionista;
import server.session.FormularioEnwipRemote;

public class CargarTipoInversionistaFormulario extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CargarTipoInversionistaFormulario() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      System.out.println("service()...");
		...
		Vector<Tipo_Inversionista> tInversionista;
		tInversionista=str.getTipoInversionista();
		

		StringBuilder objSecciones;
         // Crea un array de objetos JSON con cada tema de la tabla
            objSecciones = new StringBuilder("[");
           for( int i=0;i<tInversionista.size();i++ )
            { objSecciones.append("{\"Tipo\":\"");
            objSecciones.append(tInversionista.get(i).getNombre());
            objSecciones.append("\"},");
            }            
