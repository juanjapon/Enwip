...
public class GuardarInversionPreoperativa extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private EnwipSesionRemote esr;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GuardarInversionPreoperativa() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      //Recupera todos los nombre de parámetros recibidos en la petición.
             ...
             {   
        		 ...
        		 Vector<String>  datos=new Vector();
        		   ...
        		   System.out.println("tamaño matriz "+(nombres.size())/2);
        	        String inversionPreoperativa[][]=new String[(nombres.size())/2][3];
        	        for(int i=0,j=0;i<nombres.size();i+=2,j++){
        	        	
        	        	String aux[];
        	        	aux=nombres.get(i).split("_");
        	        	System.out.println("la i ="+i+" la j ="+j+" tipo ="+aux[0]);
        	        	inversionPreoperativa[j][0]=aux[1];
        	        	inversionPreoperativa[j][1]=datos.get(i);
        	        	inversionPreoperativa[j][2]=datos.get(i+1);
        	        ...
        	        for(int i=0;i<inversionPreoperativa.length;i++){
        	        	for(int j=0;j<inversionPreoperativa [i].length;j++){
        	        		System.out.print(" "+inversionPreoperativa[i][j]);
        	        	}
        	        	System.out.println();
        	        }
             ...
             String planId=request.getParameter("planId");
                 ...
                 out.print(esr.guardarInversionPreoperativa(inversionPreoperativa,planId));
                 out.close();
